...
```json
{
  ...
  "jwkthumbprint_hash_func_supported": [
    "SHA256"
  ]
}
```
...
* `jwk_thumbprint_hash_func_values_supported`
    * OPTIONAL. A JSON array containing a list of the hash functions that Self-Issued OP uses to compute thumbprint of a JWK used to sign the ID Token, when JWK Thumbprint Subject Syntax Type is used for the `sub` Claim. When this value is not provided, default is SHA-256.
