...
In traditional OpenID Connect, the ID token is signed by the OP, which is identified by the `iss` claim. The RP uses this identifier to obtain the key material to validate the ID token's signature. This signature ensures the data is attested by the OP the RP trusts for that purpose and it also is an attestation of what service created the ID token (since both are the same entity). In the self-issued case, those roles fall appart. The issuer of the ID token is the user since it is signed with a private key under the user's control. The ID token's signature can no longer be used to crypthographically validate the software or service that created the ID token. Setting the `iss` to the URL that might have been used to discover the SIOP therefore would not add any security. From a conceptually perspective, the issuer is the user thus this specification requires the `iss` value to be set to the user identifier conveyed in the `sub` claim. 
